...
void *jacobi(void *args){
    // Jacobi iteration
    // This loop will end if a fixed number of maximum iterations have completed
    ...
    //calculate begin and end of the thread
    int begin = id * chunk;
    int end = begin + chunk;
...
int main(int argc, char *argv[]){
    ...
    for(int i = 0; i < num_threads; i++){
        t_id[i] = i;
        pthread_create(&threads[i], NULL, jacobi, (void *) &t_id[i]);
    }
